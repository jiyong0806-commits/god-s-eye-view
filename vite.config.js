      ...
      const sessionConfig = {
        session: {
          ...
          instructions: [
            ...
            'The user speaks Korean. Understand Korean map commands and answer aloud in Korean by default, with short natural confirmations.',
    ...
    parameters: {
      ...
      properties: {
        layerId: {
          type: 'string',
          description:
            'Common-name mapping for the non-obvious ids: space mission(s) → rocket-launches; fires/wildfires/active fires → local-firms (NASA FIRMS); ships/vessels/boats → ais-live-vessels; undersea/submarine cables → telegeography-submarine-cables; datacenters → local-datacenters; dams → local-dams; bikes/bike share → bikeshare; street traffic/congestion/cars/vehicles → traffic; people/pedestrians/crowds → people-activity; temperature/weather/heat → temperature; traffic cameras/security cameras/CCTV → cctv; internet radio/stations → radio. Korean: 사람/보행자/군중/유동인구 → people-activity; 차/자동차/차량/교통 → traffic; 온도/날씨/열감 → temperature; 재난/화재/산불 → local-firms plus earthquakes; CCTV/카메라/감시카메라 → cctv.',
          enum: [
            'flights',
            'military',
            'earthquakes',
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      allowedHosts: (!env.HOST || env.HOST === '0.0.0.0' || env.HOST === '::')
        ? true
        : localAllowedHosts,
      fs: {
    ...
    build: {
      ...
      rollupOptions: {
        input: {
          ...
          event: path.resolve(__dirname, 'event/index.html'),
        },
        output: {
          manualChunks(id) {
            if (id.includes('/node_modules/cesium/')) return 'cesium';
            if (id.includes('/node_modules/satellite.js/')) return 'satellite';
            if (id.includes('/node_modules/@mapbox/vector-tile/')
              || id.includes('/node_modules/pbf/')) return 'map-vector';
            if (id.includes('/node_modules/egm96-universal/')) return 'terrain-grid';
            return undefined;
          },
        },
      },

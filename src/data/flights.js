    ...
    try {
      ...
      if (response.status === 429) {
        ...
        const waitMs = Number.isFinite(retrySeconds) && retrySeconds > 0
          ? Math.max(BACKOFF_INTERVAL, Math.min(10 * 60_000, Math.ceil(retrySeconds * 1000)))
          : BACKOFF_INTERVAL;
        console.warn(`[Data:Flights] Rate limited, backing off ${Math.ceil(waitMs / 1000)}s`);
        _backoff = true;
        _retryAt = nowMs + waitMs;
        _lastError = `${responseSource || 'Aircraft feed'} rate limited`;
        return;
      }

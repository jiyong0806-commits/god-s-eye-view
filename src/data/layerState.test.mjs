...
test('startup restore runs at most three enabled layers concurrently', async () => {
  const ids = ['earthquakes', 'emergency-vehicles', 'emissions', 'historic-ruins'];
  ...
  let peak = 0;
  let started = 0;
  const hooks = Object.fromEntries(ids.map((id) => [id, {
    init: async () => {
      started += 1;
      active += 1;
      peak = Math.max(peak, active);
      if (started === 3) firstThree.resolve();
      if (started === 4) fourth.resolve();
      await gates[id].promise;
      active -= 1;
      return true;
    },
  }]));
  const manager = productionManager(hooks);
  const state = createDefaultLayerState();
  state.enabledLayerIds = ids;
  const coordinator = new LayerStateCoordinator(manager, shareSink(), { storage: memoryStorage() });
  const restore = coordinator.start({ shareLayerState: state });
  ...
  assert.equal(peak, 3);
  for (const id of ids.slice(1)) gates[id].resolve();
  await restore;
  coordinator.destroy();
});

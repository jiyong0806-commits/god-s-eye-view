...
test('unconfigured public stream surfaces its real backend error', () => {
  assert.equal(
    deriveAisFeedError({ status: 'unsupported', error: 'AISStream persistent backend is not configured on this host' }, 0),
    'AISStream persistent backend is not configured on this host',
  );
});

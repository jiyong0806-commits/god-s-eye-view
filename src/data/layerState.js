...
const OPTION_GROUPS = Object.freeze({
  ...
  cctv: Object.freeze([
    enumOption('coverageMode', 'c', 'viewshed', ['off', 'on', 'viewshed'], {
      off: '0',
      on: '1',
      viewshed: 'v',
    }),
...
export const LAYER_STATE_REGISTRY = Object.freeze([
  ...
  Object.freeze({ id: 'emergency-vehicles', token: '0', disposition: 'enabled-only' }),
  Object.freeze({ id: 'emissions', token: '1', disposition: 'enabled-only' }),
  Object.freeze({ id: 'flights', token: 'f', disposition: 'enabled+options', optionOwner: 'flights' }),
  Object.freeze({ id: 'historic-ruins', token: '2', disposition: 'enabled-only' }),
  ...
  Object.freeze({ id: 'people-activity', token: 'o', disposition: 'enabled-only' }),
  ...
  Object.freeze({ id: 'safety-zones', token: '3', disposition: 'enabled-only' }),
  Object.freeze({ id: 'satellites', token: 's', disposition: 'enabled+options', optionOwner: 'satellites' }),
  Object.freeze({ id: 'shelters-hospitals', token: '4', disposition: 'enabled-only' }),
  Object.freeze({ id: 'speed-cameras', token: '5', disposition: 'enabled-only' }),
  Object.freeze({ id: 'telegeography-submarine-cables', token: 'u', disposition: 'enabled-only' }),
  Object.freeze({ id: 'temperature', token: 'h', disposition: 'enabled-only' }),
  Object.freeze({ id: 'traffic', token: 't', disposition: 'enabled-only' }),
  Object.freeze({ id: 'airports-transit', token: '6', disposition: 'enabled-only' }),
  Object.freeze({ id: 'volcano-ash-flood', token: '7', disposition: 'enabled-only' }),
  Object.freeze({ id: 'weather-radar', token: '8', disposition: 'enabled-only' }),
  Object.freeze({ id: 'lightning', token: 'j', disposition: 'enabled-only' }),
  Object.freeze({ id: 'aurora-solarwind', token: 'k', disposition: 'enabled-only' }),
  Object.freeze({ id: 'iss-live', token: 'l', disposition: 'enabled-only' }),
  Object.freeze({ id: 'ocean-currents', token: 'n', disposition: 'enabled-only' }),
  Object.freeze({ id: 'borders-labels', token: 'p', disposition: 'enabled-only' }),
  Object.freeze({ id: 'war-map', token: 'v', disposition: 'enabled-only' }),
  Object.freeze({ id: 'street-photos', token: 'y', disposition: 'enabled-only' }),
  Object.freeze({ id: 'language-global', token: '9', disposition: 'enabled-only' }),
]);
...
export class LayerStateCoordinator {
  ...
  async _restoreSelectedState(origin) {
    ...
    try {
      ...
      const acquireRestoreSlot = async () => {
        if (activeRestores < 3) {
          activeRestores += 1;
          return;
        }
        await new Promise((resolve) => restoreWaiters.push(resolve));
      };
      const releaseRestoreSlot = () => {
        const next = restoreWaiters.shift();
        if (next) next();
        else activeRestores -= 1;
      };
      const settled = await Promise.allSettled(LAYER_STATE_REGISTRY.map(async (entry) => {
        ...
        if (targetEnabled) await acquireRestoreSlot();
        if (this._destroyed || controller?.signal.aborted) {
          if (targetEnabled) releaseRestoreSlot();
          return {
            layerId: entry.id,
            targetEnabled,
            origin,
        ...
        const restore = Promise.resolve().then(() => this.dataManager.restoreLayerState(entry.id, {
        ...
        if (!targetEnabled) return restore;
        void restore.then(releaseRestoreSlot, releaseRestoreSlot);
        return restore;
      }));

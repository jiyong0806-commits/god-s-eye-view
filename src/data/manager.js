...
const LAYER_DISPLAY_NAMES_KO = Object.freeze({
  flights: '항공기',
  military: '군용 항공기',
  earthquakes: '지진·재난',
  satellites: '위성',
  'rocket-launches': '우주 발사',
  traffic: '차량·교통',
  'people-activity': '사람·유동인구',
  temperature: '온도',
  cctv: 'CCTV',
  radio: '라디오',
  bikeshare: '공유 자전거',
  'ais-live-vessels': '선박',
  'military-installations': '군사 시설',
  'military-awareness': '군사 상황',
  'local-datacenters': '데이터센터',
  'local-dams': '댐',
  'local-firms': '화재·산불',
  'weather-radar': '날씨 레이더',
  'safety-zones': '지역 안전도',
  'shelters-hospitals': '대피소·병원',
  'speed-cameras': '속도 카메라',
  'airports-transit': '공항·철도·지하철',
  'emergency-vehicles': '긴급 차량',
  emissions: '배출량·대기오염',
  'historic-ruins': '폐허·역사 장소',
  'volcano-ash-flood': '화산재·홍수 피해',
  lightning: '낙뢰',
  'aurora-solarwind': '오로라·태양풍',
  'iss-live': 'ISS 위치·라이브',
  'ocean-currents': '해류·조석',
  'borders-labels': '국경·지역명',
  'war-map': '분쟁 지도',
  'street-photos': '거리 사진',
  'language-global': '언어·현지화',
});

const LAYER_PANEL_GROUPS = Object.freeze({
  flights: '항공·우주',
  military: '항공·우주',
  satellites: '항공·우주',
  'rocket-launches': '항공·우주',
  'ais-live-vessels': '항공·우주',
  'iss-live': '항공·우주',
  'aurora-solarwind': '항공·우주',

  traffic: '교통·도시',
  'people-activity': '교통·도시',
  bikeshare: '교통·도시',
  temperature: '교통·도시',
  'speed-cameras': '교통·도시',
  'airports-transit': '교통·도시',
  'emergency-vehicles': '교통·도시',
  ...
  'safety-zones': '재난·환경',
  emissions: '재난·환경',
  'volcano-ash-flood': '재난·환경',
  lightning: '재난·환경',

  cctv: '감시·안전',
  'shelters-hospitals': '감시·안전',
  'military-installations': '감시·안전',
  'military-awareness': '감시·안전',
  'war-map': '감시·안전',
  'street-photos': '감시·안전',

  'local-datacenters': '인프라·통신',
  'local-dams': '인프라·통신',
  'ocean-currents': '인프라·통신',
  'language-global': '인프라·통신',
  'borders-labels': '지형·역사',
  'historic-ruins': '지형·역사',
  radio: '미디어',
});

const LAYER_PANEL_GROUP_ORDER = Object.freeze([
  '항공·우주',
  '교통·도시',
  '재난·환경',
  '감시·안전',
...
function layerPanelSortKey(layer) {
  const group = layerPanelGroup(layer);
  const groupIndex = LAYER_PANEL_GROUP_ORDER.indexOf(group);
  return `${String(groupIndex < 0 ? 99 : groupIndex).padStart(2, '0')}:${displayLayerName(layer)}`;
}

function displayLayerName(layer) {
  return LAYER_DISPLAY_NAMES_KO[layer?.id] || layer?.name || '레이어';
}

function layerPanelGroup(layer) {
  return LAYER_PANEL_GROUPS[layer?.id] || '기타';
}
...
export function layerFeedState(stats = {}) {
  ...
  if (['restricted', 'rate-limited'].includes(status) || [401, 403, 429, 451].includes(state.httpStatus)) return 'restricted';
...
export class DataLayerManager {
  ...
  _normalizedStats(entry) {
    ...
    return {
      ...
      lastUpdate: null,
      ...moduleStats,
      ...providerHealth(entry.module.id),
  ...
  _renderToggles() {
    ...
    let lastGroup = '';
    const visibleLayers = this.getAll()
      .filter((layer) => layer.showInTogglePanel)
      .sort((a, b) => layerPanelSortKey(a).localeCompare(layerPanelSortKey(b), 'ko'));
    for (const layer of visibleLayers) {
      ...
      name.textContent = displayLayerName(layer);
  ...
  _buildMetaText(layer) {
    ...
    const http = stats.httpStatus ? ` · HTTP ${stats.httpStatus}` : '';
    ...
    if (lifecycleState === 'enabling' || lifecycleState === 'disabling') {
      return `${lifecycleState === 'enabling' ? '켜는 중' : '끄는 중'} · ${source}`;
    }
    ...
    if (presentedError) {
      if (typeof stats.retryInSec === 'number' && stats.retryInSec > 0) {
        return `${stateLabel} · ${source}${http} · ${presentedError} · ${Math.ceil(stats.retryInSec)}초 뒤 재시도`;
      }
      return `${stateLabel} · ${source}${http} · ${presentedError}`;
    }
    const ago = stats.lastUpdate ? this._timeAgo(stats.lastUpdate) : '기록 없음';
    if (stats.loading) {
  ...
  _syncToggleButton(button, layer) {
    ...
    button.textContent = transitioning
      ? (layer.lifecycleState === 'enabling' ? '켜는 중' : '끄는 중')
      : (uncertain ? '확인 중' : (layer.enabled ? FEED_STATE_LABELS[feedState] : '꺼짐'));
    button.setAttribute('aria-label', `${displayLayerName(layer)}: ${button.textContent}`);
  }

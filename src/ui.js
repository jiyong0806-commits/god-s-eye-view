...
export class StyleManager {
  ...
  constructor(viewer, { mapStackController = null } = {}) {
    ...
    this._mapSourceSubtitle = document.getElementById('map-source-subtitle');
  ...
  _syncCctvSourceBadge(activeCamera, enabled) {
    ...
    if (!enabled || !activeCamera) {
      this._cctvSourceBadge.textContent = '소스 · 알 수 없음';
  ...
  _resetCctvCalibration() {
    ...
    this._showToast('CCTV 보정을 초기화했습니다');
  }
  ...
  _syncCctvCalReadout(enabled, activeCamera) {
    ...
    if (this._cctvAdjustBtn) {
      ...
      this._cctvAdjustBtn.textContent = adjustOn ? '조정 켜짐' : '조정';
  ...
  async _toggleCctvEnabled(forceState) {
    if (!this._dataManager || !this._dataManager.layers?.has('cctv')) {
      this._showToast('CCTV 레이어를 사용할 수 없습니다');
      return false;
    }
  ...
  _calBadgeLabel(badge) {
    switch (badge) {
      case 'calibrated': return '보정됨';
      case 'curated': return '검수됨';
      case 'raw-prior': return '원본 추정';
      default: return '--';
    }
  }
  ...
  _renderCctvState(state) {
    ...
    if (this._cctvEnableBtn) {
      ...
      this._cctvEnableBtn.textContent = enabled ? 'CCTV 켜짐' : 'CCTV 꺼짐';
    }
    ...
    if (this._cctvAutoHopBtn) {
      ...
      this._cctvAutoHopBtn.textContent = autoHop ? '자동 이동 켜짐' : '자동 이동 꺼짐';
    ...
    if (this._cctvProjectionBtn) {
      ...
      this._cctvProjectionBtn.textContent = showProjection ? '투영 켜짐' : '투영 꺼짐';
    ...
    if (this._cctvMeta) {
      if (activeCamera) {
        const provider = activeCamera.sourceLabel || activeCamera.provider || '설정된 소스';
        ...
        const projLabel = state?.showProjection !== false ? '모니터' : '꺼짐';
        this._cctvMeta.textContent = `${activeCamera.city} · 방위 ${Math.round(activeCamera.headingDeg)}° · 시야 ${Math.round(activeCamera.fovDeg)}° · 거리 ${Math.round(activeCamera.rangeM)}m · ${projLabel}${calBadge ? ` · ${calBadge}` : ''} · ${provider}${statusMsg}`;
      } else if (cameras.length > 0) {
        this._cctvMeta.textContent = enabled
          ? `${cameras.length}개 카메라 로드됨 · 카메라를 클릭하면 활성화`
          : `${cameras.length}개 카메라 로드됨 · CCTV를 켜면 활성화`;
      } else {
        this._cctvMeta.textContent = 'CCTV를 켜면 주변 카메라 교차로를 불러옵니다';
      }
    }
    ...
    this._typeCctvSummary(state?.summary || 'CCTV를 켜면 카메라 기반 장면 요약을 시작합니다.');
  }
  ...
  _typeCctvSummary(text) {
    ...
    const nextText = String(text || '').trim() || '요약이 없습니다.';

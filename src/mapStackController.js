...
export function photorealUnavailableReason(hasCredentials) {
  if (hasCredentials) return 'Google 3D 사용 불가 — API 키 유효성, Map Tiles API 활성화, 결제, 제한 설정을 확인하세요';
  return `${keySetupRequirement('google-maps')} — 또는 Cesium ion 토큰으로 ion 호스팅 경로를 사용하세요`;
}

export const MAP_STACKS = [
  {
    id: 'photoreal',
    label: 'Google 포토리얼 3D',
    shortLabel: '3D',
    kind: 'photoreal',
    requiresIon: false,
  },
...
const NASA_BLUE_MARBLE_CREDIT = 'NASA GIBS / Blue Marble';
...
export class MapStackController {
  constructor(viewer, {
    ...
    this._activeId = googleTileset ? initialStack : 'nasa-blue-marble';
    ...
    if (!this.getStack(this._activeId) || !this.isStackAvailable(this._activeId)) {
      this._activeId = googleTileset ? 'photoreal' : 'nasa-blue-marble';
    }
  }
  ...
  _unavailableReason(stack) {
    ...
    return `${stack?.label || '이 지도 소스'}를 사용할 수 없습니다`;
  }

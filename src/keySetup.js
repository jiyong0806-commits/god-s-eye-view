...
export function keySetupChipLabel(status) {
  ...
  return missing > 0 ? `키 설정 · ${missing}개 남음` : '키 설정 완료';
}
...
function buildRow(documentRef, key) {
  ...
  tier.title = key.tier === 'metered' ? '종량제 — 결제 계정과 한도 설정 필요' : '무료 키 — 가입 후 붙여넣기';
  ...
  if (key.clientExposed) {
    ...
    exposed.textContent = '브라우저 노출';
    exposed.title = '이 키는 브라우저에서 쓰입니다. Google Cloud에서 리퍼러/API 제한을 반드시 거세요.';
    head.append(exposed);
  }
  if (external) {
    ...
    badge.textContent = '외부 설정됨';
    badge.title = '환경변수, Keychain, 런처에서 들어온 값입니다. 설정된 곳에서 바꾸세요.';
    head.append(badge);
  }
  ...
  get.textContent = key.set ? '관리 ↗' : '키 받기 ↗';
  head.append(get);
  ...
  if (!external) {
    ...
    if (key.managed === 'file') {
      ...
      remove.title = `${key.title} 키를 저장된 설정에서 삭제`;
...
export async function initKeySetup({ documentRef = globalThis.document, fetchImpl } = {}) {
  ...
  const submitUpdates = async (updates, doneVerb) => {
    ...
    try {
      ...
      say(`${doneVerb} ${storeLabel()}. 서버 재시작 중 — 페이지가 자동으로 새로고침됩니다.`);
    } catch (error) {
      say(`저장 실패: ${error?.message || error}`);
    } finally {
      busy = false;
  ...
  const onApply = async () => {
    ...
    if (!Object.keys(updates).length) {
      say('먼저 키를 하나 이상 붙여넣으세요.');
      return;
    }
    await submitUpdates(updates, '저장 위치:');
  };
  ...
  rowsHost?.addEventListener('click', (event) => {
    ...
    const ok = typeof globalThis.confirm !== 'function'
      || globalThis.confirm('저장된 설정에서 이 키를 삭제할까요?');
    if (!ok) return;
    void submitUpdates(

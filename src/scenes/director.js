...
function sceneLabel(value) {
  ...
  return SCENE_LABELS[key] || SCENE_LABELS[key.toLowerCase()] || key;
}
  ...
  const scenes = scenesRaw
    .map((scene, sceneIdx) => {
      ...
      return {
        ...
        title: scene?.title || `장면 ${sceneIdx + 1}`,
        shots,
      };
    })
...
export class SceneDirector {
  ...
  _renderShotList() {
    ...
    if (!scene || scene.shots.length === 0) {
      ...
      empty.textContent = '아직 샷이 없습니다. 샷 캡처로 현재 화면을 저장하세요.';
    ...
    for (const shot of scene.shots) {
      ...
      label.addEventListener('dblclick', () => {
        const nextTitle = window.prompt('샷 이름', shot.title);
        if (!nextTitle) return;
      ...
      meta.textContent = `${sceneLabel(style)} · ${sceneLabel(mode)} · ${shot.durationSec.toFixed(1)}초 + ${shot.holdSec.toFixed(1)}초`;
  ...
  _createScene() {
    const sceneName = window.prompt('새 장면 이름', `장면 ${this._project.scenes.length + 1}`);
    if (!sceneName) return;

    const scene = {
      id: uid('scene'),
      title: sceneName.trim() || `장면 ${this._project.scenes.length + 1}`,
      shots: [],
    };
  ...
  _deleteSelectedScene() {
    ...
    const ok = window.confirm(`"${scene.title}" 장면과 모든 샷을 삭제할까요?`);
    if (!ok) return;
  ...
  captureShot() {
    ...
    if (!camera) {
      this._updateStatus('샷 캡처 실패: 카메라가 아직 준비되지 않았습니다');
      return;
    }

    const shot = normalizeShot({
      id: uid('shot'),
      title: `샷 ${scene.shots.length + 1}`,
    ...
    this._saveProject();
    this._renderShotList();
    this._updateStatus(`캡처됨: ${scene.title} / ${shot.title}`);
  }
  ...
  updateSelectedShot() {
    ...
    if (!shot) {
      this._updateStatus('먼저 샷을 선택하세요');
      return;
    }
    ...
    this._saveProject();
    this._renderShotList();
    this._updateStatus(`갱신됨: ${scene.title} / ${shot.title}`);
  }
  ...
  deleteShot(sceneId, shotId) {
    ...
    const ok = window.confirm(`"${shot.title}" 샷을 삭제할까요?`);
    if (!ok) return;
  ...
  async loadShot(sceneId, shotId, { flyDuration = 2.2 } = {}) {
    ...
    this._updateStatus(`불러옴: ${scene.title} / ${shot.title}`);
    this._updateRuntime('');
  }
  ...
  _claimCameraOwnership() {
    ...
    if (claimed === false) {
      this._updateStatus('카메라 사용 불가: 먼저 조종석 모드를 종료하세요');
      return false;
    }
    return true;
  }
  ...
  async startScene(sceneId, { single = false } = {}) {
    ...
    if (!queue.length) {
      this._updateStatus('실행할 샷이 없습니다');
    ...
    try {
      ...
      for (let idx = 0; idx < queue.length; idx++) {
        ...
        this._updateStatus(`실행 중 ${idx + 1}/${queue.length}: ${scene.title} / ${shot.title}`);
      ...
      if (!token.cancelled) {
        this._setProgress(1);
        this._updateStatus('장면 실행 완료');
        this._logEvent('scene_run_complete', {});
      }
    } catch (error) {
      this._updateStatus(`오류: ${error.message || '실행 실패'}`);
  ...
  async importProjectFile(file) {
    try {
      ...
      this._renderSceneSelect();
      this._renderShotList();
      this._updateStatus(`가져옴: ${file.name}`);
    } catch {
      this._updateStatus('가져오기 실패: JSON 형식이 올바르지 않습니다');
    }
  }

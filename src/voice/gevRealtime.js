...
export class GevRealtimeController {
  ...
  async start({ pushToTalk = false } = {}) {
    ...
    this.syncCostUi();
    this.setStatus('connecting', '마이크 권한 요청 중');
    ...
    try {
      ...
      dataChannel.addEventListener('open', () => {
        const detail = this.pushToTalkMode
          ? (this.pushToTalkKeyHeld ? '스페이스를 놓으면 전송' : '스페이스를 누른 채 말하기')
          : 'Ask or command';
  ...
  bindPushToTalkShortcut() {
    ...
    this.shortcutKeyDownHandler = (event) => {
      ...
      if (this.isActive()) {
        ...
        if (this.status === 'listening') this.setStatus('listening', '스페이스를 놓으면 전송');
      } else {
  ...
  releasePushToTalkKey() {
    ...
    if (this.status === 'listening') this.setStatus('listening', '스페이스를 누른 채 말하기');
  ...
  async handleRealtimeEvent(event) {
    ...
    if (payload.type === 'error') {
      if (payload.error?.code === 'conversation_already_has_active_response') {
        ...
        this.setStatus('listening', '질문하거나 명령하세요');
        return;
      }
  ...
  setStatus(status, detail) {
    ...
    const primaryDetail = status === 'error'
      ? '음성 사용 불가'
      : (resolvedDetail || (status === 'idle' ? '음성 대기' : '음성 활성'));
    ...
    if (this.ui.errorDetail) {
      this.ui.errorDetail.textContent = status === 'error'
        ? (resolvedDetail || '음성 세션을 시작하지 못했습니다.')
        : '';
    }
  ...
  updateVoiceButtonLabel() {
    ...
    this.ui.buttonLabel.textContent = '마이크';
  ...
  async startPendingRadioHandoff() {
    ...
    if (this.dc?.readyState === 'open' && !this.userTurnPending) {
      this.setStatus('listening', '라디오를 시작하지 못했습니다');
...
export function resolveVoiceControlHint(pushToTalkMode, pushToTalkKeyHeld) {
  return pushToTalkMode && pushToTalkKeyHeld
    ? '스페이스를 놓으면 전송'
    : '스페이스를 누른 채 말하기 · 마이크 클릭으로 음성 켜기/끄기';
}
  ...
  if (!root) {
    ...
    root.innerHTML = `
      <div class="gev-voice-heading">
        <div class="gev-voice-kicker">AI 음성</div>
        <div id="gev-voice-status">꺼짐</div>
        <div class="gev-voice-cost">
          <button id="gev-voice-tier" class="gev-voice-tier-btn" type="button" aria-pressed="false" title="음성 모델 등급 - 다음 세션부터 적용">STD</button>
          <span id="gev-voice-cost-value" class="gev-voice-cost-value" data-level="ok" title="예상 세션 비용">~$0.00</span>
        </div>
      </div>
      <button id="gev-voice-button" type="button" aria-label="음성 명령 - 스페이스를 누른 채 말하거나 클릭해서 켜고 끄기" aria-describedby="gev-voice-help">
      ...
      <div class="gev-voice-readout">
        <div id="gev-voice-detail">음성 대기</div>
      </div>
      <div id="gev-voice-help" class="gev-voice-help-tray" role="tooltip">
        <span class="gev-voice-help-kicker">음성 명령</span>
        <span class="gev-voice-help-detail">스페이스를 누른 채 말하기 · 마이크 클릭으로 음성 켜기/끄기</span>
      </div>
      <div class="gev-voice-error-tray" role="alert" aria-live="assertive">
        <div class="gev-voice-error-header">
          <span>음성 시스템 오류</span>
          <button class="gev-voice-error-dismiss" type="button">닫기</button>
        </div>
        <div id="gev-voice-error-detail"></div>
        <div class="gev-voice-error-hint">마이크 권한과 네트워크 연결을 확인한 뒤 다시 시도하세요.</div>
      </div>
    `;

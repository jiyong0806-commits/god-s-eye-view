...
test('overlay backing stores cap high-DPR devices at 2x', () => {
  const env = installMockEnvironment({ width: 400, height: 300, dpr: 3 });
  initWorldOverlay(env.viewer);
  setOverlayEntries('high-dpr-probe', [selectedEntry('probe')]);
  env.postRender.raise();
  const canvas = env.document.getElementById('world-overlay-canvas');
  const detectionSurface = env.document.getElementById('world-overlay-detection-surface');
  assert.equal(canvas.width, 800);
  assert.equal(canvas.height, 600);
  assert.equal(detectionSurface.width, 800);
  assert.equal(detectionSurface.height, 600);
  assert.ok(env.ctx.calls.some((call) => call[0] === 'setTransform' && call[1] === 2));
  env.cleanup();
});
